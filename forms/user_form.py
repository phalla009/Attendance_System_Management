...
class UserTypeForm(FlaskForm):
    ...
    TypeName = StringField('Type Name', validators=[DataRequired(), Length(max=50)])


class ContactNoForm(FlaskForm):
    ContactNumber = StringField('Contact Number', validators=[DataRequired(), Length(max=20)])


class AddressForm(FlaskForm):
    Home = StringField('Home', validators=[Length(max=50)])
    Street = StringField('Street', validators=[Length(max=100)])
    ProvinceID = SelectField('Province', coerce=int, validators=[DataRequired()])
    DistrictID = SelectField('District', coerce=int, validators=[DataRequired()])
...
# 👉 សម្រាប់ Teacher Form (មាន Subject ប៉ុន្តែគ្មាន Group និង Class)
